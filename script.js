...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function checkOptionsVisibility() {
        if (gameState.currentScreen === 'battle') {
            ...
            if (battleUI && optionsContainer) {
                // Calculate if all options are visible
                const optionsHeight = optionsContainer.getBoundingClientRect().height;
                const battleUIHeight = battleUI.getBoundingClientRect().height;
                const controlsHeight = document.querySelector('.control-menu').getBoundingClientRect().height;
                const questionHeight = document.querySelector('.question-box').getBoundingClientRect().height;
                
                // If options don't fit, add a class to enable scrolling
                if (optionsHeight + controlsHeight + questionHeight > battleUIHeight) {
                    battleUI.classList.add('needs-scrolling');
                } else {
                    battleUI.classList.remove('needs-scrolling');
                }
            }
        }
    }
    
    // Add event for when the screen changes
    ['resize', 'orientationchange'].forEach(event => {
        window.addEventListener(event, checkOptionsVisibility);
    });
    
    // Check after rendering a question
    const originalRenderQuestion = renderQuestion;
    renderQuestion = function() {
        originalRenderQuestion();
        setTimeout(checkOptionsVisibility, 100); // Check after DOM update
    };
    
    // Add touch event handling to ensure scrolling works on iOS
